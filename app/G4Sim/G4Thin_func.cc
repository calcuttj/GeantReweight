...
int main(int argc, char * argv[]){
  ...
  std::vector< double > inel_points = inel_bias_par.get< std::vector< double > >("Points");
  std::vector< double > inel_vals = inel_bias_par.get< std::vector< double > >("Vals");
  std::vector< std::pair< double, double > > inel_input;

  for( size_t ib = 0; ib < inel_points.size(); ++ib ){
    inel_input.push_back( std::make_pair(inel_points[ib], inel_vals[ib]) );
  }

  G4ReweightInter inelasticBias(inel_input);

  fhicl::ParameterSet el_bias_par = pset.get< fhicl::ParameterSet >("Elastic"); 
  std::vector< double > el_points = el_bias_par.get< std::vector< double > >("Points");
  std::vector< double > el_vals = el_bias_par.get< std::vector< double > >("Vals");
  std::vector< std::pair< double, double > > el_input;

  for( size_t ib = 0; ib < el_points.size(); ++ib ){
    el_input.push_back( std::make_pair(el_points[ib], el_vals[ib]) );
  }

  G4ReweightInter elasticBias(el_input);
